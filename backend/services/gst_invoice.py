...
def build_gst_invoice(row: dict[str, Any], company: dict[str, Any], index: int = 0, user_id: int | None = None) -> dict[str, Any]:
    ...
    invoice_total = 0.0
    gst_rate = 0.0
    same_state = _normalize_state(company.get("supplier_state")) == _normalize_state(buyer_state)
    for item in items:
        ...
        item_gst_rate = _positive_or_zero(stock.get("gst_rate"), "GST rate")
        if item_gst_rate <= 0:
            raise GstInvoiceError("GST rate is missing for this stock item")
        quantity = _positive_float(item.get("quantity"), "quantity")
        if item.get("price_is_inclusive_total"):
            tax = calculate_gst_totals_from_inclusive_total(
                invoice_total=_positive_float(item.get("price"), "price"),
            ...
            rate = _positive_float(item.get("rate") if item.get("rate") is not None else item.get("price"), "rate")
        ...
        taxable_amount = round(taxable_amount + float(tax["taxable_amount"]), 2)
        cgst_amount = round(cgst_amount + float(tax["cgst_amount"]), 2)
        sgst_amount = round(sgst_amount + float(tax["sgst_amount"]), 2)
        igst_amount = round(igst_amount + float(tax["igst_amount"]), 2)
        invoice_total = round(invoice_total + float(tax["invoice_total"]), 2)
        gst_rate = item_gst_rate if not gst_rate else gst_rate
        inventory_entries.append(
            {
                ...
                "HSNCode": str(stock.get("hsn_code") or "").strip(),
...
def _row_items(row: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if raw_items:
        if not isinstance(raw_items, list):
            raise GstInvoiceError("items must be a list")
        items = raw_items
    else:
        items = [
            {
                ...
                "price_is_inclusive_total": row.get("price_is_inclusive_total"),
            }
        ]
    if not items:
        raise GstInvoiceError("At least one item is required")
    normalized = []
    for index, item in enumerate(items):
        product_name = str((item or {}).get("product_name", "")).strip()
        if not product_name:
            raise GstInvoiceError(f"items[{index}].product_name is required")
        normalized.append(
            {
                "product_name": product_name,
                "price": (item or {}).get("price"),
                "rate": (item or {}).get("rate"),
                "quantity": _positive_float((item or {}).get("quantity"), "quantity"),
                "price_is_inclusive_total": bool((item or {}).get("price_is_inclusive_total")),
            }
        )
    return normalized
        ...
        "items": [
            {
                ...
                "price": round(float(item.get("price") or item.get("rate") or 0), 2),
                "rate": round(float(item.get("rate") or 0), 2) if item.get("rate") is not None else None,
                "quantity": round(float(item.get("quantity") or 0), 4),
                "price_is_inclusive_total": bool(item.get("price_is_inclusive_total")),
            }
            for item in items
        ],
        "buyer_gstin": str(row.get("buyer_gstin", "")).strip().upper(),
